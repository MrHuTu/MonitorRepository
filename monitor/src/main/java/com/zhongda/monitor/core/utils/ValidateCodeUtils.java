...
public class ValidateCodeUtils {
	...
	private String randString = "0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz";// 随机生成字符串的取值范围

	private int width = 100; // 图片宽度
	private int height = 47; // 图片高度
	private int StringNum = 4; // 验证码图片中随机产生字符的数量
	private int lineSize = 19; // 干扰线数量
	...
	private Font getFont() {
		return new Font("Fixedsys", Font.ITALIC, 20); // 名称、样式、磅值
	}
	...
	private String drawString(Graphics g, String randomString, int i) {
		...
		g.setColor(new Color(random.nextInt(101), random.nextInt(111), random
				.nextInt(121)));// 设置颜色
		String randChar = String.valueOf(getRandomChar(random
				.nextInt(randString.length())));
		randomString += randChar; // 组装
		g.translate(random.nextInt(3), random.nextInt(3));
		g.drawString(randChar, 16 * i, 23);
		return randomString;
	}
